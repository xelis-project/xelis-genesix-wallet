...
impl HistoryPageFilter {
    pub fn options<'a>(&'a self) -> Result<TransactionFilterOptions<'a>> {
        ...
        let asset = match self.asset_hash.as_ref() {
            Some(asset_hash) => Some(Cow::Owned(Hash::from_hex(&asset_hash).context("Invalid asset")?)),
            None => None,
        };
        ...
        Ok(TransactionFilterOptions {
            ...
            accept_coinbase: match self.address { Some(_) => false, None => self.accept_coinbase, },
            accept_burn: match self.address { Some(_) => false, None => self.accept_burn, },
